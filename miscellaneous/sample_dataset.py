...
from script.utils import PDF, calculate_pdf
...
if __name__ == "__main__":
    ...
    # pdf_type = "MULTIVARIATE_1254"
    pdf_type = "EXPONENTIAL_06"

    prova_NEW = PDF(default=pdf_type)
    ...
    # seaborn
    sns.set()
    plt.plot(prova_NEW.test_X, prova_NEW.test_Y, label="True PDF")

    # Genera l'istogramma
    # plt.hist(samples_array, bins=32, density=True, alpha=0.7, color="grey", label="Samples Data")
    # plt.scatter(trainingX, trainingY, label="Training data")
    # plt.scatter(prova_NEW.training_X, prova_NEW.training_Y, alpha=0.5)
    # plt.legend()
    plt.xlabel("X")
    plt.ylabel("Probability Density")
    # plt.title("Istogramma della Distribuzione di Probabilità")
    plt.grid(True)
    plt.savefig(f"pdf_{pdf_type}.png")
    plt.show()
