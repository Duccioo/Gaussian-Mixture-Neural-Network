...
from testing_parzen import parzen_window
from testing_kn_nn import kn_nn_estimation
...
def test_and_log(
    ...
    if model is not None and pdf_predicted is None:
...
def main():
    ...
    parser.add_argument("--parzen_window", type=float, default=0.2)
    parser.add_argument("--k1_value", type=int, default=4)
    ...
    parzen_window_h = args.parzen_window
    K1 = args.k1_value
    ...
    # predict the pdf with Parzen window
    pdf_predicted_parzen = [parzen_window(point, x_training, parzen_window_h) for point in x_test]
    test_and_log(
        pdf_predicted=pdf_predicted_parzen,
        X=x_test,
        y_true=y_test,
        pdf_type=pdf,
        pdf=pdf.params,
    ...
    kn = int(K1 * np.sqrt(len(x_training)))

    # predict the pdf with KN_NN
    pdf_predicted_knn = [kn_nn_estimation(x_training, point, kn) for point in x_test]
    test_and_log(
        pdf_predicted=pdf_predicted_knn,
        X=x_test,
        y_true=y_test,
        pdf_type=pdf,
        pdf=pdf.params,
