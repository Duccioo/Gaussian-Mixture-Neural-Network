...
class Summary:
    ...
    def __init__(
        ...
        if overwrite:
            self.experiment_dir = check_base_dir(path_subtype, experiment + " " + self.id_experiment)
        else:
            self.experiment_dir = check_base_dir(
                unique_dir(os.path.join(path_subtype, experiment + " " + self.id_experiment))
            )
    ...
    def plot_pdf(
        ...
        if Y_target is not None:
            ...
            # Unisci le due liste in una lista di tuple
            combined = list(zip(X_train, Y_target))

            # Ordina la lista di tuple in base ai valori di x
            combined.sort(key=lambda pair: pair[0])

            # Dividi nuovamente le tuple ordinate nelle liste x e y
            X_train, Y_target = zip(*combined)
